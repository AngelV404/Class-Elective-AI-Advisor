...
class share_advisor_frame(ctk.CTkFrame):
    def __init__(self, parent,email,on_share=None,on_cancel=None):
        ...
        title=ctk.CTkLabel(self,text='Detail',font=list_font)
        title.grid(row=0,column=0,sticky='nwe',pady=(0,10))

        self.email=email
        
        label=ctk.CTkLabel(self, text='Course will be shared with advisor', font=list_font)
        email_label=ctk.CTkLabel(self,text=email,font=list_font)
        label.grid(row=1,column=0,sticky='nswe')
        email_label.grid(row=2,column=0,sticky='nswe')
        
        share_button=createButton(self,'Share',command=self.handle_share)
        share_button.configure(width=200)
        share_button.grid(row=3,column=0,pady=(40,20))
        
        
        cancel_button=createButton(self,'Cancel',self.handle_cancel)
